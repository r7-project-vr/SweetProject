...
namespace ASerialDataStruct {
    struct ASerialData {
        uint8_t command;
        uint8_t data[_ASERIAL_MAX_DATA_NUM];
        uint8_t data_num;
    }; //ASerialのデータ構造体
...
private:
    ...
    //================
    uint8_t m_mode;   //動作モード設定
    uint8_t m_device_id;        //デバイスID
    uint8_t m_device_ver;       //デバイスVer
    uint8_t m_target_device_id; //ターゲットデバイスID
    bool m_add_flag;        //加算フラグ
    bool m_read_packet;     //packet読み取りフラグ(true中はパケット読み取り中)
    ...
    bool m_error_flag;        //何らかのエラーでパケットを無視するフラグ
    uint16_t m_lase_error_code;    //最後にエラーになったときのエラーコード
    bool m_connection_state;        //コントローラモード時の接続ステータス
